...
app.get('/api/me', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json({ id: me.id, email: me.correo, nombre: me.nombre, telefono: me.telefono });
  } catch (err) { next(err); }
});
...
app.put('/api/me', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json({ id: u.id, email: u.correo, nombre: u.nombre, telefono: u.telefono });
  } catch (err) { next(err); }
});

// ---- Mascotas (ABMC) ----
// ✅ ahora con cumpleDia / cumpleMes (sin año)
const mascotaCreateSchema = z.object({
  nombre: z.string().min(1, 'El nombre es obligatorio'),
  raza: z.string().optional(),
  sexo: z.string().optional(),
  edadMeses: z.coerce.number().int().positive().optional(),
  ...
  try {
    ...
    const mascota = await prisma.mascota.update({
      ...
      data: {
        ...
        photoPath: b.photoPath ?? existing.photoPath
      }
    });
    res.json(mascota);
  } catch (err) { next(err); }
...
app.delete(['/api/me/mascotas/:id','/api/me/dogs/:id'], requireAuth, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const existing = await prisma.mascota.findUnique({ where: { id } });
    if (!existing || existing.duenioId !== req.user.id) {
